...
import io.github.randomusert.mods.tincraft.api.crafting.ShapelessCraftingInput;
...
public class BaseItemStackHandler extends ItemStackHandler {
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (slot >= 0 && slot < this.stacks.size()) {
                ITEM_STACK_CODEC.parse(lookup.createSerializationContext(NbtOps.INSTANCE), item)
                        .resultOrPartial(error -> LogHelper.LogError("Tried to load invalid item: '{}'"))
    ...
    public CraftingInput toShapelessCraftingInput() {
        return new ShapelessCraftingInput(this.stacks).asVanilla();
    }
    ...
    public CraftingInput toShapelessCraftingInput(int startIndex, int endIndex) {
        return new ShapelessCraftingInput(this.stacks.subList(startIndex, endIndex)).asVanilla();
    }
